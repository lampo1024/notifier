var notifier = {
    ...
    renderMessage: function (data, callback) {
        if (!callback) {
            appendMessage(data);
        }
        else {
...
notifier.init("http://localhost:5125/hub/notifier");
async function start() {
    try {
        await notifier.connection.start();
        await sendSystemMessage("已成功连接到消息服务器");
        console.log("connection:", notifier.connection.connectionId);
    } catch (err) {
        console.error(err);
...
function sayHi() {
    console.log("Hi from datacenter");
}